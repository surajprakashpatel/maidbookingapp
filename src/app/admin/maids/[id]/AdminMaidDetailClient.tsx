...
import { subscribeToMaidById, updateMaidApprovalStatus, deleteMaid } from '@/lib/services/maidService';
import { Maid } from '@/lib/types';
import { useApp } from '@/lib/app-context';
import { formatINR, getApprovalStatusClass, getApprovalStatusLabel } from '@/lib/utils';
import { CheckCircle, XCircle, Shield, Camera, MapPin, Phone, Mail, Eye, AlertTriangle, Loader, UserX, Trash2 } from 'lucide-react';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter } from '@/components/ui/dialog';
import { Button } from '@/components/ui/button';

export default function AdminMaidDetailClient({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (!id) return;
    setLoading(true);
    const unsub = subscribeToMaidById(id, (liveMaid) => {
      setMaid(liveMaid);
      setLoading(false);
    });
    return () => unsub();
  ...
  const handleDeleteMaid = async () => {
    if (!maid) return;
    setDeleting(true);
    const success = await deleteMaid(maid.id);
    if (success) {
      showToast('info', 'Maid Record Deleted', `${maid.name}'s profile was removed from database.`);
      router.push('/admin/maids');
    } else {
      showToast('error', 'Delete Failed', 'Could not delete maid record.');
    }
    setDeleting(false);
        ...
        <div style={{ background: 'white', borderRadius: 'var(--radius-xl)', padding: '20px', border: '1px solid var(--border-light)' }}>
          ...
          {showRejectionInput ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              >
                <Trash2 className="size-4 text-red-600" /> Delete Maid Record
              </Button>
            </div>
          )}
        </div>
          ...
          <DialogContent className="max-w-sm">
            <DialogHeader>
              <DialogTitle className="text-red-600 flex items-center gap-2">
                <Trash2 className="size-5" /> Delete Maid Application
              </DialogTitle>
              <DialogDescription className="text-xs pt-2">
                Are you sure you want to delete <strong>{maid.name}</strong> from Firestore? This action cannot be undone.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter className="flex gap-2 sm:justify-end pt-2">
              ...
              <Button variant="destructive" size="sm" onClick={handleDeleteMaid} disabled={deleting}>
