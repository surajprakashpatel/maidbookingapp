  ...
  'fixed z-50 gap-4 bg-white p-6 shadow-xl transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-200 data-[state=open]:duration-300',
  {
    variants: {
      side: {
        ...
        bottom:
          'inset-x-0 bottom-0 border-t border-[var(--border)] rounded-t-3xl data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom max-h-[85vh] overflow-y-auto',
...
>(({ side = 'bottom', className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {side === 'bottom' && (
        <div className="mx-auto -mt-3 mb-4 h-1.5 w-12 rounded-full bg-[var(--gray-300)]" />
      )}
      {children}
      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-lg p-1 opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-[var(--primary-600)]">
        <X className="h-4 w-4" />
